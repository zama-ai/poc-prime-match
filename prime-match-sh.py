...
inputset = [
    (
        np.random.randint(MINIMUM_ORDER_QUANTITY, MAXIMUM_ORDER_QUANTITY, size=(NUMBER_OF_SYMBOLS*2,)),
        np.random.randint(MINIMUM_ORDER_QUANTITY, MAXIMUM_ORDER_QUANTITY, size=(NUMBER_OF_SYMBOLS*2,)),
    )
    for _ in range(1000)
]
...
for i in range(NUMBER_OF_SYMBOLS):
    ...
    sample_bank_order_quantities[idx] = np.random.randint(MINIMUM_ORDER_QUANTITY, MAXIMUM_ORDER_QUANTITY)
    sample_client_order_quantities[idx] = np.random.randint(MINIMUM_ORDER_QUANTITY, MAXIMUM_ORDER_QUANTITY)
...
print("\tResult Orders:")
for c1_order, c2_order , result in zip(sample_bank_order_quantities[:15], sample_client_order_quantities[:15], simulated_matches[:15]):
...
print("\tResult Orders:")
for c1_order, c2_order , result in zip(sample_bank_order_quantities[:15], sample_client_order_quantities[:15], executed_matches[:15]):
    print(f"\t\t{c1_order}\t{c2_order}\t->\t{result}")
print()
