...
class Order:
    ...
    @staticmethod
    def random(blacklist: Set[OrderSymbol]) -> "Order":
        order_type = random.choice(list(OrderType))
        order_symbol = random.choice(list(set(OrderSymbol) - blacklist))
...
def prime_match(
    ...
    with fhe.tag("calculating-matching-order-quantity"):
        matching_quantity = np.minimum(bank_order_quantities.reshape(-1, 1), client_order_quantities)
...
inputset = [
    (
        ...
        np.array(
            [
                int(symbol)
                for symbol in random.sample(list(OrderSymbol), NUMBER_OF_BANK_ORDERS)
            ]
        ),
        np.random.randint(
        ...
        np.array(
            [
                int(symbol)
                for symbol in random.sample(list(OrderSymbol), NUMBER_OF_CLIENT_ORDERS)
            ]
        ),
        np.random.randint(
            MINIMUM_ORDER_QUANTITY,
...
blacklist = set()
for i in range(NUMBER_OF_BANK_ORDERS):
...
blacklist = set()
for i in range(NUMBER_OF_CLIENT_ORDERS):
    order = Order.random(blacklist)
    blacklist.add(order.order_symbol)
